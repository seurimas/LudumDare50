...
pub struct Sequence<R> {
    nodes: Vec<Box<dyn PoweredFunction<World = R> + Send + Sync>>,
    index: Option<usize>,
}

impl<R> Sequence<R> {
    pub fn new(nodes: Vec<Box<dyn PoweredFunction<World = R> + Send + Sync>>) -> Self {
...
mod tests {
    ...
    fn test_sequence_success() {
        let nodes: Vec<Box<dyn PoweredFunction<World = ()> + Send + Sync>> = vec![
