use crate::ai::powered::*;

pub struct Repeat<R> {
    node: Box<dyn PoweredFunction<World = R> + Send + Sync>,
    runs: usize,
    runs_left: usize,
}

impl<R> Repeat<R> {
    pub fn new(node: Box<dyn PoweredFunction<World = R> + Send + Sync>, runs: usize) -> Self {
        Repeat {
            node,
            runs,
            runs_left: runs,
...
impl<R> RepeatUntilFail<R> {
    pub fn new(node: Box<dyn PoweredFunction<World = R> + Send + Sync>) -> Self {
